...
const META: Record<string, StrategyMeta> = {
  ...
  ema_adx_trend_follow: {
    label: { zh: 'EMA + ADX 趋势跟随', en: 'EMA + ADX Trend Follow' },
    description: {
      zh: '用 EMA 方向与 ADX 强度共同确认趋势信号。',
      en: 'Uses EMA trend alignment and ADX strength to confirm trend entries.',
    },
    category: { zh: '机构模板', en: 'Institutional' },
    logic_summary: {
      zh: '快 EMA 高于慢 EMA 且 ADX 达标时维持多头。',
      en: 'Stays long when fast EMA leads and ADX is above threshold.',
    },
  },
  volatility_contraction_breakout: {
    label: { zh: '波动收缩突破', en: 'Volatility Contraction Breakout' },
    description: {
      zh: '先等待波动收缩，再跟踪放量突破。',
      en: 'Waits for volatility contraction before breakout with participation.',
    },
    category: { zh: '机构模板', en: 'Institutional' },
    logic_summary: {
      zh: '波动收缩 + 突破确认后入场。',
      en: 'Enters only after volatility compression and breakout confirmation.',
    },
  },
  keltner_atr_breakout: {
    label: { zh: 'Keltner ATR 突破', en: 'Keltner ATR Breakout' },
    description: {
      zh: '基于 EMA 中轴与 ATR 通道识别趋势突破。',
      en: 'Detects trend breakouts with EMA centerline and ATR channels.',
    },
    category: { zh: '机构模板', en: 'Institutional' },
    logic_summary: {
      zh: '突破上轨入场，跌回中轴离场。',
      en: 'Enter above upper band and exit below centerline.',
    },
  },
  rsi_trend_pullback: {
    label: { zh: 'RSI 趋势回撤', en: 'RSI Trend Pullback' },
    description: {
      zh: '在上升趋势中寻找 RSI 回撤后的再启动机会。',
      en: 'Finds pullback RSI zone entries within broader uptrend.',
    },
    category: { zh: '机构模板', en: 'Institutional' },
    logic_summary: {
      zh: '趋势过滤 + RSI 回撤区间确认。',
      en: 'Combines trend filter with RSI pullback window.',
    },
  },
}
...
const FIELD_META: Record<string, StrategyFieldMeta[]> = {
  ...
  ema_adx_trend_follow: [
    { key: 'fast_ema', min: 2, step: 1, label: { zh: '快 EMA', en: 'Fast EMA' } },
    { key: 'slow_ema', min: 5, step: 1, label: { zh: '慢 EMA', en: 'Slow EMA' } },
    { key: 'adx_window', min: 5, step: 1, label: { zh: 'ADX 窗口', en: 'ADX Window' } },
    { key: 'adx_threshold', min: 5, step: 0.5, label: { zh: 'ADX 阈值', en: 'ADX Threshold' } },
  ],
  volatility_contraction_breakout: [
    { key: 'lookback', min: 20, step: 1, label: { zh: '长波动窗口', en: 'Long Vol Window' } },
    { key: 'contraction_window', min: 5, step: 1, label: { zh: '收缩窗口', en: 'Contraction Window' } },
    { key: 'breakout_window', min: 5, step: 1, label: { zh: '突破窗口', en: 'Breakout Window' } },
    { key: 'volatility_threshold', min: 0.2, step: 0.01, label: { zh: '收缩阈值', en: 'Contraction Threshold' } },
  ],
  keltner_atr_breakout: [
    { key: 'ema_window', min: 5, step: 1, label: { zh: 'EMA 窗口', en: 'EMA Window' } },
    { key: 'atr_window', min: 5, step: 1, label: { zh: 'ATR 窗口', en: 'ATR Window' } },
    { key: 'atr_multiplier', min: 0.5, step: 0.1, label: { zh: 'ATR 倍数', en: 'ATR Multiplier' } },
  ],
  rsi_trend_pullback: [
    { key: 'trend_window', min: 10, step: 1, label: { zh: '趋势窗口', en: 'Trend Window' } },
    { key: 'rsi_period', min: 2, step: 1, label: { zh: 'RSI 周期', en: 'RSI Period' } },
    { key: 'pullback_floor', min: 10, step: 1, label: { zh: '回撤下限', en: 'Pullback Floor' } },
    { key: 'pullback_ceiling', min: 20, step: 1, label: { zh: '回撤上限', en: 'Pullback Ceiling' } },
  ],
}
...
export const FALLBACK_STRATEGIES: StrategyDefinition[] = [
  ...
  },
  {
    ...
    default_params: { lookback: 60, contraction_window: 10, breakout_window: 20, volatility_threshold: 0.75 },
  },
  {
  ...
  },
  {
    ...
    default_params: { trend_window: 60, rsi_period: 14, pullback_floor: 38, pullback_ceiling: 52 },
  },
]
