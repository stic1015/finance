...
const pageTitle = computed(() => {
  if (route.name === 'stock') return '个股研究'
  if (route.name === 'strategy-lab') return '策略实验室'
  return '市场总览'
})
  ...
  <div class="app-shell">
    <header class="topbar page-shell">
      <div class="brand-lockup">
        <span class="eyebrow">量化投资研究台</span>
        <h1>{{ pageTitle }}</h1>
      </div>
      <nav>
        <RouterLink to="/">市场总览</RouterLink>
        <RouterLink to="/stocks/HK.00700">个股研究</RouterLink>
        <RouterLink to="/strategy-lab">策略实验室</RouterLink>
      </nav>
