...
const hotList = computed(() => [...filteredWatchlist.value].sort((a, b) => Math.abs(b.change_percent) - Math.abs(a.change_percent)).slice(0, 5))
const movers = computed(() => [...filteredWatchlist.value].sort((a, b) => b.change_percent - a.change_percent).slice(0, 5))
        ...
        <SectionPanel :title="localeStore.t('overview.dataTrust')" :subtitle="localeStore.t('common.marketOverview')">
          <div class="metrics-grid">
            ...
            <MetricTile
              ...
              :value="systemStore.health?.executor_mode === 'thread_pool' ? 'Thread Pool' : 'Process Pool'"
            />
            <MetricTile
